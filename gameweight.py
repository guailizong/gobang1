...
class GameWeight(QWidget):
    ...
    def initui(self):
        ...
        self.goback_button=MyButton('source/返回按钮_hover.png',
                                    ...
                                    parent=self)
        self.goback_button.move(660,150)
        ...
        self.start_button = MyButton('source/开始按钮_hover.png',
                                      ...
                                      parent=self)
        self.start_button.move(650, 250)
        ...
        self.regret_button = MyButton('source/悔棋按钮_hover.png',
                                      ...
                                      parent=self)
        self.regret_button.move(650, 350)
    ...
    def show_win(self,color):
        if color=='black':
            #黑棋胜利图片显示
            self.winlable.setPixmap('source/黑棋胜利.png')
            self.winlable.show()
            ...
            #白棋胜利图片显示
            self.winlable.setPixmap('source/白棋胜利.png')
            self.winlable.show()
            self.winlable.move(100, 100)
            # 图片在窗口最上层显示
            self.winlable.raise_()
    ...
    def mouseReleaseEvent(self, a0: QtGui.QMouseEvent):
        ...
        #调用reverse_to0_position方法返回棋盘落子位置
    ...
    #将棋盘交点坐标转化为落子坐标
    def reversed_to_coordinate(self,position):
        #(0,0)=>(50,50)
        #落子坐标=坐标原点+30*交点坐标
        x=50+30*position[0]
        y=50+30*position[1]
    ...
    def down_chess(self,position,color):
        ...
        coorx,coory=self.reversed_to_coordinate(position)
        print(coorx,coory)
        #设置棋子位置,将棋子中心落在棋盘交点
